@extends('frontend.user.layouts.auth-request')
@section('title', lang('Verify Your Email Address', 'user'))
@section('content')
    <div class="vr__sign__form vr__register my-5">
        <div class="vr__sign__header">
            ...
            @if (!session('email_change'))
                <p class="h3 mb-2">{{ lang('Thanks for getting started with', 'user') }}
                    {{ $settings['website_name'] . '!' }}</p>
                <p class="mb-2">
                    {{ lang('We need a little more information to complete your registration, including a confirmation of your email address.', 'user') }}
            ...
            @else
                <p class="h3 mb-2">{{ lang('Verify Your New Email Address', 'user') }}.</p>
                <p class="mb-2">
                    {{ lang('Since you have changed your email address, we need to verify that it is really your email', 'user') }}
                    <strong>{{ userAuthInfo()->email }}</strong>.
                </p>
                <p class="mb-3">
                    {{ lang("Please follow the instruction that we sent to your email, if you didn't receive the email click resent to get a new one.", 'user') }}
        ...
        <div class="sign-body">
            <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                @csrf
                <button type="submit" class="btn btn-secondary btn-lg me-2">{{ lang('Resend', 'user') }}</button>
            </form>
            <button href="#" class="btn btn-outline-primary btn-lg" data-bs-toggle="modal"
                data-bs-target="#emailModal">{{ lang('Change Email', 'user') }}</button>
        </div>
    </div>
                    ...
                    <div class="modal-body">
                        <div class="form-floating my-3">
                            <input type="email" name="email" id="email" class="form-control"
                                placeholder="{{ lang('Email address', 'forms') }}" value="{{ userAuthInfo()->email }}"
                                required>
                            <label>{{ lang('Email address', 'forms') }}</label>
                        </div>
                        <div class="d-flex justify-content-between">
                            <button type="submit" class="btn btn-secondary w-100 me-2">{{ lang('Save', 'user') }}</button>
