@if (request()->routeIs('admin.users.edit') && $subscription)
    <a href="{{ route('admin.subscriptions.edit', $subscription->id) }}" class="btn btn-blue mb-3 btn-lg w-100"><i
            class="far fa-gem me-2"></i>{{ __('Edit Subscription') }}</a>
@endif
...
<div class="custom-card card mb-3">
    <div class="custom-list-group list-group list-group-flush">
        ...
        <a href="{{ route('admin.users.bookings', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('admin.users.bookings')) active @endif">
            <span>{{ __('Booking') }}</span>
            <span><i class="fas fa-angle-right"></i></span>
        </a>
        <a href="{{ route('admin.users.boat', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('admin.users.boat')) active @endif">
            <span>{{ __('Boat') }}</span>
            <span><i class="fas fa-angle-right"></i></span>
        </a>
