...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <form id="vironeer-submited-form" action="{{ route('admin.pushNotification.update', $notification->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-lg-4">                      
                                ...
                                <select name="app_name[]" id="app_name" class="form-select select2" required>
                                    <option value=""></option>
                                    <option value="Mobile" {{ (in_array("Mobile", $app_name_arr))?"selected":"" }}>Mobile App</option>
                                    <option value="Transport" {{ (in_array("Transport", $app_name_arr))?"selected":"" }}>Transport</option>
                                    <option value="Academic" {{ (in_array("Academic", $app_name_arr))?"selected":"" }}>Academic</option>
                                </select>
                            </div>
                            <div class="col-lg-4">                      
                                ...
                                <select name="template_id" id="template_id" class="form-select select2" onChange="setTemplateContent()">
                                    ...
                                    @if(count($notificationList) > 0)
                                        @foreach($notificationList as $temp)
                                            <option value="{{ $temp->id }}" data-id="{{ $temp->message }}" {{ ($temp->id == $notification->template_id)?"selected":"" }}>{{ $temp->title }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                                ...
                                <select name="redirection" id="redirection" class="form-select select2">
                                    ...
                                    @if(count($redirectionList) > 0)
                                        @foreach($redirectionList as $res)
                                            <option value="{{ $res->id }}" {{ ($res->id == $notification->redirection)?"selected":"" }}>{{ $res->title }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-4">                      
                                ...
                                <select name="student_id[]" id="student_id" class="form-select select2" multiple>
                                    ...
                                    <!-- <option value="all" @if($notification->student_id =='0') selected @endif >All</option>  -->
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-8">
                                ...
                                <input type="text" name="title" id="title" class="form-control" value="{{ $notification->title }}" maxlength="50" required>
                            </div> 
                            <div class="col-lg-3">
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-1">
                                @if(!empty($notification->image))
									<img href="{{ asset($notification->image) }}" height="100px">
								@endif
                            </div>
                            <div class="col-lg-12">      
                                <label class="form-label">Message: <span class="red">*</span></label>
                                <textarea id="message" name="message" rows="3" class="form-control">{{ $notification->message }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    @php
        $explodeArr = [];
        if($notification->student_id != 0){
            $explodeArr = explode(',', $notification->student_id);
    }
    @endphp 
<script>
    var student_id = '{{ $notification->student_id }}'; 
