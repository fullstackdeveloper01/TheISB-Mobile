...
@section('back', route('admin.knowledgeBase.index'))
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.knowledgeBase.update', $knowledgeBase->id) }}" method="POST"
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="create_slug" class="form-control"
                                value="{{ $knowledgeBase->title }}" required />
                        </div>
                        <div class="mb-0">
                            <label class="form-label">{{ __('Description') }} : <span class="red">*</span></label>
                            <textarea name="content" id="content" rows="10" class="form-control" required>{{ $knowledgeBase->content }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="vironeer-file-preview-box mb-3 bg-light p-5 text-center">
                            <div class="file-preview-box mb-3">
                                <img id="filePreview" src="{{ asset($knowledgeBase->image) }}" class="rounded-3 w-100"
                                    height="160px">
                            </div>
                        ...
                        <div class="mb-0">
                            ...
                            <textarea name="short_description" rows="6" class="form-control" placeholder="{{ __('50 to 200 character at most') }}" maxlength="200" required>{{ $knowledgeBase->short_description }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
