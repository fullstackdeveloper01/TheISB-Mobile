...
class GeneralController extends Controller
{
    ...
    {
        $mainSection = MainSection::where('id', 1)->first();
        $appVersion = MainSection::where('id','<>', 1)->get()->toArray();
        return view('backend.settings.general.index', ['mainSection' => $mainSection, 'appVersion' => $appVersion]);
    }
    ...
    public function update(Request $request)
    {
         ...
         **************************************/
        $mainSection = MainSection::where('id', 1)->first();
        if ($request->has('main_section_icon')) {
            $main_section_icon = vImageUpload($request->file('main_section_icon'), 'images/main-section/', '110x110');
            $mainSection->icon = $main_section_icon;
        }
        if($request->main_section_heading != ''){
            $mainSection->heading = $request->main_section_heading;
        }
        if($request->main_section_line_color != ''){
            $mainSection->line_color = $request->main_section_line_color;
        }
        if($request->main_section_file_type != ''){
            $mainSection->file_type = $request->main_section_file_type;
            if($request->main_section_file_type == 'color'){
                $mainSection->file_value = $request->main_section_file_value;
            }
            else{
                if ($request->has('main_section_image')) {
                    $main_section_image = vImageUpload($request->file('main_section_image'), 'images/main-section/', '110x110');
                    $mainSection->file_value = $main_section_image;
                }
            }
        }
        $mainSection->save();
        ...
        for($a=2; $a<7; $a++){
            $appSection = MainSection::where('id', $a)->first();
            $appSection->heading = $postData['app_heading_'.$a];
            $appSection->app_version = $postData['app_version_'.$a];
            $appSection->save();
        }
