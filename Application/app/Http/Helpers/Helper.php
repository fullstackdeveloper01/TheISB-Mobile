...
function studentName($sid){
    ...
    if($sid == '0'){
        ...
        $sidArr = explode(',',$sid);
        $studentList = User::select('firstname', 'lastname')->whereIn('student_id', $sidArr)->get();
        foreach($studentList as $s){
            array_push($s_name_array, trim($s->firstname).' '.trim($s->lastname));
        }
        $sname = implode(', ', $s_name_array);
    }
    return $sname;
}
...
function studentSection($section){
    ...
    if(!empty($sectionArr)){
        foreach($sectionArr as $s){
            array_push($new_section, 'A'.$s);
        }
        return implode(', ', $new_section);
    }
    else{
        return 'All';
    }
...
function uploadChecker()
{
    ...
    //     if (request()->segment(1) != 'install' && env('VIRONEER_SYSTEMSTATUS')) {
    //         $LCNPath = base_path('vendor/phpValid/.LCN');
    //         $MITPath = base_path('vendor/laravel/framework/.MIT');
    //         if (file_exists($LCNPath) && file_exists($MITPath)) {
    //             $explodeLCNLine = explode('-vr-', file($LCNPath)[0]);
    //             $explodeMITLine = explode('-', file($MITPath)[0]);
    //             if ($explodeMITLine[0] != systemInfo()['name']) {
    //                 return false;
    //             }
    //             if (decrypt($explodeLCNLine[2]) != systemInfo()['name']) {
    //                 return false;
    //             }
    //             if (str_replace('www.', '', parse_url(url('/'))['host']) != decrypt($explodeLCNLine[1])) {
    //                 return false;
    //             }
    //         } else {
    //             return false;
...
function sendNotification($fcm_token, $title, $message, $url = null,$redirection_url = '')
{
    if($fcm_token!='')
    {
        ...
        $headers = array
        (
            'Authorization: key=AAAA3hN1EDs:APA91bG1iBCIqGWRu1HTxG30HIQuFuCXKXaiMvItzYpnLX-PAWXrl5nBPqAKVW-zXyjBPMFAatqz-QhkMzQUBmTcrLOb3ZWVuDA4GMg1Ly9i_lWd8ZImZjvmVFah_UFMkv-UTrxwhuls',
...
/*******************************************************************************************
 * @Function: Student push notification store
*******************************************************************************************/
function notificationStore($request, $type=2){
    if($type == 1)
    {
      ...
      $notify = UserNotificationLogs::create([
                    'notification_id'   => $request['notification_id'],
                    'device_id'         => $request['device_id'],
                    'student_id'        => $request['student_id'],
                    'user_id'           => $request['user_id'],
