    ...
    selectCategory.on('change', function() {
        ...
        $.ajax({
            ...
            success: function(resp) {
                let length_ =  resp.data.length;
                var resps = resp.data;
                if(resp.data.length > 0)
                {
                    var category_html = '';
                    for(var c=0; c<length_; c++){
                        category_html += '<option value="'+resps[c].id+'">'+resps[c].name+'</option>';
                    }
                    $('#sub_category_id').html(category_html);
                }
                else{
